...
Enfin, nous présenterons les \textbf{formes rythmiques}, <flo>une représentation
...
\begin{enumerate}
    ...
    \item si c’est un charley fermé joué au pied $\Rightarrow$ la note sera un
        silence.
\end{enumerate}
La deuxième règle sera soumise au cadre imposé par certaines \textbf{formes rythmiques}
...
être adaptés en fonction des différentes \textbf{formes rythmiques} reconnues (voir la
section \ref{systemes_methodes}). Par exemple, pour les \textbf{formes rythmiques} jazz,
...
Il aurait fallu expliquer le rôle des grammaires (automates) en 3.3(devenu 3.4)}
...
le style et la signature rythmique sont récupérables par les noms des fichiers MIDI, 
...
Par conséquent, les \textbf{motifs} des \textbf{formes rythmiques} devront être recherchés sur l’input \textit{(fichiers MIDI)} 
...
\textbf{gammes} des \textbf{formes rythmiques} seront utilisés pour la
...
<dam>assez dur à suivre ici, essayes de prendre de la hauteur et juste de t'appuyer sur l'exemple (sans trop le décrire en détail) pour dire quelles sont les difficultés et les choix de séparation de voix et comment un algorithme pourrait faire ces choix.</dam>
Ici, le \textbf{forme rythmique} est construit sur un modèle rock en 4/4 : after-beat
sur les 2 et 4 avec un choix de répartition des cymbales type fast-jazz. La
\textbf{forme rythmique} est constituée par défaut du \textbf{motif} rd/pf/cc (voir
\ref{pitchs_instru}) et d’un \textbf{gamme} jouée à la grosse caisse. La première ligne
...
La figure \ref{afro_latin} montre un exemple minimaliste de \textbf{forme rythmique} afro-latin \cite{system_drums}. 
Ce \textbf{forme rythmique} doit être écrite sur trois voix car la voix centrale est
...
\textbf{forme rythmique} dans la section \ref{reecriture_guidee}.

Les \textbf{gammes} qui accompagnent les \textbf{motifs} d’un \textbf{forme rythmique} étayent toutes les combinaisons d’un \textbf{forme rythmique} et elles permettent, combinées avec le \textbf{motif} d’un \textbf{forme rythmique}, de définir les règles de simplification propres à celui-ci.

Voici les différentes étapes à suivre :
\begin{itemize}
	\item Pour chaque \textbf{gamme} du \textbf{forme rythmique}, faire un arbre de rythme représentant la \textbf{gamme} combinée avec le \textbf{motif} du \textbf{forme rythmique} ;
...
Pour chacune des conditions suivantes, une suite<dam>pas défini</dam> de la
