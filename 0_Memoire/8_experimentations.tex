...
\section{Discussion}
Le choix de travailler avec lilypond au lieu de musescore est pertinent car
une modélisation de la batterie avec une notation cohérente a pu être établi
ainsi que de nombreux exemple de scripts lilypond recouvrant plusieurs style
d’évènements de batterie qui s’avérer très utiles dans des contextes binaires
ou ternaire.

De plus, le fait que lilypond utilise du texte (scripts) pour générer des
partitions en pdf pourrait permettre d’élaborer un système d’évaluation en
utilisant le traitement de données textuelles.

On pourrait par exemple, configurer l’\textit{ouput} de qparse au format
lilypond pour ensuite comparer une partition de référence transcrite
manuellement avec lilypond avec la sortie. cette comparaison pourrait être
effectuée avec la commande «~diff~».




qui peuvent déjà servir de référence pour la transcription automatique et la
...
de la TAB avec qparse. pertinence de l’ensemble des choix

BILAN DES DIFFÉRENTES AVANCÉS QUI ONT ÉTÉ FAITES



fichier lilypond de référence avec la sortie 
disponibilité d’un set de notation de type agostini. C’est la seule application qui me permettait d’écrire la notation de la batterie
exactement comme je le souhaitais.

Le passage au polyphonique a bien fonctionné car nous avons obtenu des arbres
correct en sortie
...
L’ensemble des travaux finis, notamment les avancées réalisées
dans ce travail
la pertinence des choix qui ont été faits et les moyens d’évaluer les
résultats obtenus.
 Nous avons améliorer le système de quantification de
...
Dans cette section, nous discuterons sur la  Nous ferons un .
\begin{itemize}
    \item 
