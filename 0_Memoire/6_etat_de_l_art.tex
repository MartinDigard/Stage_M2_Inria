...
\florent{présenterons quelques travaux antérieurs}
...
\florent{de signaux audio}
vers la génération du MIDI \cite{AMT_for_2_Instru}. 
\florent{MIDI \textbf{non-quantifié} = performance (à expliquer)}
...
sur la différence entre les timings de performance (dont le MIDI non-quantifié est un enregistrement symbolique)
et les timing des partitions. avec 2 unités temporelles différentes (secondes et temps), en relation par tempo.}
l'estimation du tempo, la quantification du rythme, 
la classification des genres musicaux, etc…
\florent{classification des genres? ce n'est pas de la transcription!
séparation des sources oui.}

\florent{avant l'ADT, il faudrait dire 2 mots sur les techniques utilisées (cf. survey AMT Benetos et al.)}
...
\florent{classification des évènements? la phrase semble redondante}
...
\florent{pas clair... peut–être juste mentionner les modèles probabilistes utilisés}
